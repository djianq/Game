	...
	for(;;) 
	{
		...
		for(i = 0; i < s->slot_size; ++i) 
		{
			...
			if(s->slot[hash] == NULL) 	// 找到未使用的  slot 将这个 ctx 放入这个 slot 中			
			{
				...
				handle |= s->harbor;	// harbor 用于不同主机间的通信 handle高8位用于harbor 低24位用于本机的 所以这里
				return handle;
			}
		}
		...
		skynet_free(s->slot);	// free old mem
		s->slot = new_slot;
		s->slot_size *= 2;
	}
}
...
void skynet_handle_init(int harbor) 
{
	...
	s->slot = skynet_malloc(s->slot_size * sizeof(struct skynet_context *));	// 为 skynet_ctx 分配空间
	memset(s->slot, 0, s->slot_size * sizeof(struct skynet_context *));

	rwlock_init(&s->lock);
	// reserve 0 for system
	s->harbor = (uint32_t)(harbor & 0xff) << HANDLE_REMOTE_SHIFT;	//高8位
