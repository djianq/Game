...
#define MAX_EVENT 64			//用于epoll_wait的第三个参数每次返回事件的多少
#define MIN_READ_BUFFER 64		//最小分配的读缓冲大小为了减少read的调用尽可能分配大的读缓冲区  muduo是用了reav来减少read系统调用次数的它可以准备多块缓冲区
#define SOCKET_TYPE_INVALID 0 	//无效的socket fd
...
struct socket_server 
{
	...
	int event_n;						// epoll_wait 返回的事件个数
...
static int reserve_id(struct socket_server *ss) 
{
	...
	{
		int id = ATOM_INC(&(ss->alloc_id));	//CAS原子++
		if(id < 0) // 小于0 已经最大了 说明从0开始
		{
...
static int open_socket(struct socket_server *ss, struct request_open *request, struct socket_message *result) 
{
	...
	{
		ns->type = SOCKET_TYPE_CONNECTING;	// 非阻塞套接字处于连接中应该关注它的可写事件epoll关注它是否连接成功
...
static int send_socket(struct socket_server *ss, struct request_send *request, struct socket_message *result, int priority, const uint8_t *udp_address) 
{
	...
	if(send_buffer_empty(s) && s->type == SOCKET_TYPE_CONNECTED) 
	{
		if(s->protocol == PROTOCOL_TCP) 
		{
			...
			if(n == so.sz)  // 可以把数据拷贝到内核缓冲区中
			{
				so.free_func(request->buffer);
				return -1;
			}

			append_sendbuffer(ss, s, request, n);	// add to high priority list, even priority == PRIORITY_LOW  // 将未发送的部分添加到应用层缓冲区中关注可写事件
		} 
		else 
		{
			// udp
...
static int close_socket(struct socket_server *ss, struct request_close *request, struct socket_message *result) 
{
	...
	if(!send_buffer_empty(s)) 	//关闭socket的时候 如果有未发送的数据先发送出去
	{ 
...
//判断管道是否有命名：使用select来管理没有使用epoll是为了提高命令的检测频率
...
static int report_connect(struct socket_server *ss, struct socket *s, struct socket_message *result) 
{
	...
	else 
	{
		...
		if(send_buffer_empty(s)) 	//  取消关注可写事件
		{
...
static int report_accept(struct socket_server *ss, struct socket *s, struct socket_message *result) 
{
	...
	struct socket *ns = new_fd(ss, id, client_fd, PROTOCOL_TCP, s->opaque, false); // 现在还没有把它放到epoll中，而是做了一次记录。标志了一下哪个槽被引用了
...
int socket_server_poll(struct socket_server *ss, struct socket_message *result, int *more) 
{
	for(;;) 
	{
		if(ss->checkctrl) 
		{
			if(has_cmd(ss)) 	// has_cmd内部调用select函数 判断管道是否有命令  使用select来管理 没有使用epoll是为了提高命令的检测频率
			{	
