...
-- 检查命令行参数
local SETTING_FILE = "setting_language.lua" -- eg. "setting_server.lua"
assert(SETTING_FILE, "第一个参数必须是maker的配置文件名，才能运行"..__U8T(arg[0]))
local MAKER_DIR = arg[1] -- eg. "server/setting/"
assert(MAKER_DIR, "第一个参数必须是所有maker_*.lua所在的目录，才能运行"..__U8T(arg[0]))
local OUTPUT_DIR = OUTPUTBASE..arg[2] -- eg. "server/setting/"
assert(OUTPUT_DIR, "第二个参数必须是结果数据文件输出目录，才能运行"..__U8T(arg[0]))
--
--
