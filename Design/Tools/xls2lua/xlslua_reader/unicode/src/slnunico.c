...
*	$Id: slnunico.c 10177 2009-05-06 04:53:04Z akara $
*
*	contains code from 
...
static int check_capture (MatchStateEx *ms, int l) {
	l -= '1';
...
static int capture_to_close (MatchStateEx *ms)
{
	int level = ms->level;
...
static const char *classend (MatchStateEx *ms, const char *p)
{
	switch (*p) {
	case L_ESC:
...
static unsigned deco (const MatchStateEx *ms, const char **s, const char *e)
{
	return ms->mb ? utf8_deco(s, e) : *(unsigned char*)(*s)++;
}

/* s must be < ms->src_end, p < ep */
static const char *singlematch (const MatchStateEx *ms,
...
static const char *match (MatchStateEx *ms, const char *s, const char *p);


static const char *matchbalance (MatchStateEx *ms, const char *s,
																	 const char *p) {
	if (*p == 0 || *(p+1) == 0)
...
static const char *max_expand (MatchStateEx *ms,
	const char *s, const char *p, const char *ep)
{
	const char *sp = s, *es;
...
static const char *min_expand (MatchStateEx *ms,
	const char *s, const char *p, const char *ep)
{
	do {
...
static const char *start_capture (MatchStateEx *ms, const char *s,
...
static const char *end_capture (MatchStateEx *ms, const char *s,
																	const char *p) {
...
static const char *match_capture (MatchStateEx *ms, const char *s, int l) {
	size_t len;
...
static void push_onecapture (MatchStateEx *ms, int i, const char *s,
		const char *e )
{
	if (i >= ms->level) {
...
static int push_captures (MatchStateEx *ms, const char *s, const char *e) {
	int i;
...
static void add_s (MatchStateEx *ms, luaL_Buffer *b,
...
static void add_value (MatchStateEx *ms, luaL_Buffer *b, const char *s,
	const char *e)
{
	lua_State *L = ms->L;
	switch (lua_type(L, 3)) {
...
** Open string library
*/
__declspec(dllexport) LUALIB_API  int luaopen_unicode (lua_State *L) {
