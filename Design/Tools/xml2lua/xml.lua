...
local function parseargs(s)
  ...
  s:gsub("([%w:]+)%s*=%s*([\"'])(.-)%2", function (w, _, a)
    ...
    elseif string.find(a,"%(") then
        a = string.gsub (a, "%(", "{")
        a = string.gsub (a, "%)", "}")
        a = string.gsub (a, "WALK02", "6")
        a = string.gsub (a, "WALK03", "7")
        a = string.gsub (a, "WALK04", "8")
        a = string.gsub (a, "WALK", "1")
        a = string.gsub (a, "RUN02", "14")
        a = string.gsub (a, "RUN03", "15")
        a = string.gsub (a, "RUN04", "16")
        a = string.gsub (a, "RUN", "9")
        a = string.gsub (a, "RUSH02", "22")
        a = string.gsub (a, "RUSH03", "23")
        a = string.gsub (a, "RUSH04", "24")
        a = string.gsub (a, "RUSH", "18")
        a = "b = "..a
        loadstring(a)()
        arg[w] = b
    else
        if w == "range" and not stringx.startswith(a, "melee") and not string.find(a,"@") then
            arg[w] = tonumber(a)
        else
            arg[w] = unescape(a)
        end
