...
local function accept(conf, s, fd, addr)
	...
	local ok, err, user = pcall(conf.login_handler, server, uid, secret, fd)
	-- unlock login
	user_login[uid] = nil

	if ok and err then
		ok = pcall(conf.new_agent_service, err)
		write("response 200", fd,  "200 " .. crypt.base64encode(err) .. "\n")
		write("response 200", fd,  user .. "\n")
		write("response 200", fd,  tostring(1920) .. "\n")
	else
