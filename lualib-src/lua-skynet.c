...
static int forward_cb(struct skynet_context *context, void *ud, int type, int session, uint32_t source, const void *msg, size_t sz) 
{
...
static int lcallback(lua_State *L) 
{
	...
	lua_settop(L, 1);
	lua_rawsetp(L, LUA_REGISTRYINDEX, _cb);		//t[_cb] = dispatch_message，t根据LUA_REGISTRYINDEX索引从全局表中找到的table

	lua_rawgeti(L, LUA_REGISTRYINDEX, LUA_RIDX_MAINTHREAD); //t[LUA_RIDX_MAINTHREAD]值压到栈顶 t根据LUA_REGISTRYINDEX索引从局表中找到的table
