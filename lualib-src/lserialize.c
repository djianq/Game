...
{
	struct block *pack;
	struct save_pack *next;
	char name[MAX_NAME_SIZE];
};

struct save_pack_list
{
	struct save_pack *head;
...
static void rball_init(struct read_block *rb, char *buffer) 
{
	rb->buffer = buffer;
	rb->current = NULL;
	uint8_t header[4];
...
static inline void invalid_stream_line(lua_State *L, struct read_block *rb, int line) 
{
	int len = rb->len;
...
static void push_value(lua_State *L, struct read_block *rb, int type, int cookie) 
{
	switch(type) 
	{
	case TYPE_NIL:
...
static int _dump_mem(const char *buffer, int len, int size) 
{
	// int i;
...
static void deserialize_buffer(lua_State *L, void *buffer) 
{
	struct read_block rb;
...
static int seristring(lua_State *L) 
{
	struct write_block b;
	wb_init(&b, NULL);
	pack_from(L, &b, 0);
...
static int deseristring(lua_State *L) 
{
