...
static inline const void *read_bytes(lua_State *L, struct bson_reader *br, int sz) 
{
	const void *r = br->ptr;
...
static inline const char *read_cstring(lua_State *L, struct bson_reader *br, size_t *sz) 
{
	int i;
	for(i = 0; ; ++i) 
	{
...
static void write_string(struct bson *b, const char *key, size_t sz) 
{
	bson_reserve(b, sz + 1);
...
static inline void append_key(struct bson *bs, int type, const char *key, size_t sz) 
{
	write_byte(bs, type);
