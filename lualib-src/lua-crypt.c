...
static int ldhsecret(lua_State *L) 
{
	...
	read64(L, x, y);
	xx = (uint64_t)x[0] | (uint64_t)x[1]<<32;
	yy = (uint64_t)y[0] | (uint64_t)y[1]<<32;
	if(xx == 0 || yy == 0)
...
static int ldhexchange(lua_State *L) 
{
	...
	x64 = (uint64_t)xx[0] | (uint64_t)xx[1] << 32;
	if(x64 == 0)
		return luaL_error(L, "Can't be 0");
...
static int lxor_str(lua_State *L) 
{
	...
	buffer = luaL_buffinitsize(L, &b, len1);
...
#ifdef _WIN32 
__declspec(dllexport) int luaopen_crypt(lua_State *L)
#else
